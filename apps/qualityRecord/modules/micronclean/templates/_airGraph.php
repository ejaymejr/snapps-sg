<?php
$filterArea = isset($filterArea)? $filterArea : "3l";

sfConfig::set('app_page_heading', '<span class="alignCenter"><h2>DI TEMPERATURE AND RH GRAPH FROM '.DateUtils::DUFormat('F-d, Y', $sf_params->get('tis')).' TO '.DateUtils::DUFormat('F-d, Y', $sf_params->get('tie')).'</h2></span>');


$chartID = XIDX::getInstance()->next();
        
$TITLE = isset($chartTitle) ? $chartTitle : 'Temperature and RH Graph \n'.$filterArea.' Filter Area';
$XDENSITY = 2;
$VERTGRID = 10;
...
foreach ($pager as $record) {
	if ($filterArea == $record->getFilterArea()) {
    ...
    foreach ($plots as $pl) {
        ...
        $val = 0;
        if ($filterArea == $record->getFilterArea()) { 
	        if ($pl == 'TEMPERATURE') $val = $record->getTemperature() ;
