 ...
 */
class veritasAddAction extends SnappsAction
{
    var $preCount = 0;
    ...
    public function preExecute()
    {
        ...
        if (!$this->record)
        {
            $this->record = new VeritasParticleData();
    ...
    public function execute()
    {
        if ($this->getRequest()->getMethod() == sfRequest::POST)
        {
            ...
            $this->record->setPoint5($this->_G('no_of_times_wash'));
        	$this->record->setModifiedBy($this->_U());
        	$this->record->setDateModified(DateUtils::DUNow());
        	$this->record->save();
            $data = $this->_G('customer');
            $this->_SUF('Record <b>' . $data . '</b> saved.');
            $this->redirect('micronclean/veritasSearch?hIDs[]=' . $this->record->getId());
        }
    }
