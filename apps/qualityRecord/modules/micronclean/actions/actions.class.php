...
class microncleanActions extends SnappsActions
{
    ...
    public function executeIcCalibrationEdit()
    {
        ...
        if ($rec){
            ...
            $std1_file1 = IcCalibrationFilePeer::getDataByIcCalibrationIDProofNumber($rec->getId(), 'std1_file1');
            $this->_S('std1_file1', $std1_file1);
            
            $std1_file1 = IcCalibrationFilePeer::getDataByIcCalibrationIDProofNumber($rec->getId(), 'std1_file2');
            $this->_S('std1_file2', $std1_file1);
            
            $std1_file1 = IcCalibrationFilePeer::getDataByIcCalibrationIDProofNumber($rec->getId(), 'std2_file1');
            $this->_S('std2_file1', $std1_file1);
            
            $std1_file1 = IcCalibrationFilePeer::getDataByIcCalibrationIDProofNumber($rec->getId(), 'std2_file2');
            $this->_S('std2_file2', $std1_file1);
...
//		exit();
		while($cdate <= $toDate):
			...
			if (!$data):
				...
				$data->setFoldingTableA(0);
				$data->setCheckedBy('VELU');
				$data->setVerifiedBy('HUIPING');
				$data->save();
			endif;
			echo 'Humidity: ' . $humidity . '<br>';
			echo 'Temperature: '. $temp . '<br>';	
