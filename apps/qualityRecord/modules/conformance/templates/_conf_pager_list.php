...
$desc = $record->getCustomer();
$editLink = 'Edit';
$deleteLink = 'Delete';
$editLink = link_to('Edit', 'conformance/conformanceEdit?id=' . $record->getId());
  
$deleteLink = link_to('Delete', 'conformance/conformanceDelete?id=' . $record->getId(),
                    array('confirm' => 'Record [ '.$SN.': - '.$desc . ' ]  Sure to delete this record?')); 

$viewLink = link_to('View', 'conformance/conformanceView?id=' . $record->getId());
...
<tr class="<?=$rowClass?>"
    ...
    <td class="alignCenter alignTop"  nowrap>
        <?php echo $editLink . ' | ' . $deleteLink. ' | ' . $viewLink; ?>
    </td>
    ...
    <td class="alignCenter alignTop" nowrap > <?php echo $record->getProductType() ?></td>    
    <td class="alignCenter alignTop" nowrap > <?php echo $record->getQuantity() ?></td>
    <td class="alignCenter alignTop" nowrap > <?php echo $record->getCost() ?></td>
    <td class="alignCenter alignTop" nowrap > <?php echo $record->getVpc() ?></td>
