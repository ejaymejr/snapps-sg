...
$TITLE = isset($chartTitle) ? $chartTitle : 'Particle and Temperature Graph \n Washer Number:'.$filterArea;
$XDENSITY = 2;
$VERTGRID = 10;
...
$columns = array('TEMPERATURE', 'PARTICLE COUNT');
...
foreach ($pager as $record) {
	if ($filterArea == $record->getDryerNo()) {
	    $tickLabels[] = $record->getDateTime('M j');
	    $ballonLabels[] = '';
	    
	    $plotDatas["Spec"]['values'][] = 100;
	}
    foreach ($plots as $pl) {
        if ($pl == 'Spec') continue;
        ...
        if ($filterArea == $record->getDryerNo()) { 
	        ...
	        if ($pl == 'PARTICLE COUNT') $val = $record->getParticleCount() ;
	        $val = str_replace('%', '', $val);
...
foreach ($plotDatas as $name => $tmp) {
    ...
    $displayName = $name;
    if ($name == 'Spec') {
    	$displayName = "Spec isLess 100 @ 0.3um";
    }
