...
class plasticBagAddAction extends SnappsAction
{
    ...
    public function execute()
    {
        if ($this->getRequest()->getMethod() == sfRequest::POST)
        {
            ...
            $this->record->setIcClInCm($this->_G('ic_cl_in_cm'));
            $this->record->setIcNoInCm($this->_G('ic_no_in_cm'));
            $this->record->setIcSoInCm($this->_G('ic_so_in_cm'));
            $this->record->setDateTime($this->_G('date_time')? $this->_G('date_time') : null);
            $this->record->setVendor($this->_G('vendor'));
