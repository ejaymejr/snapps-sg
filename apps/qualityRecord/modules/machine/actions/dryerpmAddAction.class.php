 ...
 */
class dryerpmAddAction extends SnappsAction
{
    var $preCount = 0;
    ...
    public function execute()
    {
        if ($this->getRequest()->getMethod() == sfRequest::POST)
        {
            ...
            $this->record->setCageShaft($this->_G('cage_shaft'));
            $this->record->setFanBearing($this->_G('fan_bearing'));
            $this->record->setFanShaft($this->_G('fan_shaft'));
            $this->record->setFanMotor($this->_G('fan_motor'));
            $this->record->setCageMotor($this->_G('cage_motor'));
            ...
            $this->redirect('machine/dryerPmSearch?hIDs[]=' . $this->record->getId());
        }
    }
